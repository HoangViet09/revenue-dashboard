...
const authApi = {
  ...
  getUserById: (userId: string): Promise<User> =>
    httpClient.get(`/auth/users/${userId}`),

  getAllUsers: (params?: {
    page?: number;
    limit?: number;
  }): Promise<PaginatedUsers> => httpClient.get("/auth/users", { params }),

  updateUser: (userId: string, data: UpdateUserData): Promise<User> =>
    httpClient.put(`/auth/users/${userId}`, data),

  changePassword: (
    userId: string,
    data: ChangePasswordData
  ): Promise<{ success: boolean }> =>
    httpClient.put(`/auth/users/${userId}/password`, data),

  deleteUser: (userId: string): Promise<{ success: boolean }> =>
    httpClient.delete(`/auth/users/${userId}`),
};
